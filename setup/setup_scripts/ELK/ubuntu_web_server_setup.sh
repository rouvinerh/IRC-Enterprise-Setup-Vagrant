...
# Output it and host it on apache server so that other machines in the intranet can use it
curl -L https://artifacts.elastic.co/downloads/beats/filebeat/filebeat-8.4.0-amd64.deb -o /var/www/html/filebeat-8.4.0-amd64.deb
dpkg -i /var/www/html/filebeat-8.4.0-amd64.deb
...
cat > /etc/filebeat/modules.d/apache.yml <<EOM
# Module: apache
